...
import { saveRace } from '../game/storage';
...
export default function RaceScreen() {
  ...
  const steer = useCallback((direction: -1 | 1) => { if (!paused) setRace(r => move(r, direction)); }, [paused]);
  ...
  useEffect(() => {
    if (paused || race.phase === 'finished') return;
    let last = performance.now();
    const timer = setInterval(() => { const now = performance.now(); const dt = Math.min(now - last, 100); last = now; setRace(r => tick(r, dt)); }, 50);
  ...
  const restart = () => { setSaveState('saving'); setPaused(false); setRace(createRace()); };
  if (race.phase === 'finished') return <View style={s.finish}><Scene paused /><ScrollView contentContainerStyle={[s.resultsContent, { paddingTop: insets.top + 38, paddingBottom: insets.bottom + 24 }]}>
    <View style={s.resultCard}><Text style={styles.eyebrow}>COSTA SOLEADA · RESULTADOS</Text><Text style={{ fontSize: 44 }}>{race.lives ? '⚑' : '↻'}</Text><Text style={styles.title}>{race.lives ? '¡Llegaste a la meta!' : 'Un intento más cerca'}</Text>
      <Text style={styles.body}>{race.lives ? 'Diez decisiones. Un nuevo recorrido en inglés.' : 'Cada error te muestra qué practicar. Vuelve a la carretera cuando quieras.'}</Text>
      ...
      <Text style={s.saveText}>{saveState === 'saved' ? 'Récord y palabras para practicar guardados en este dispositivo.' : saveState === 'saving' ? 'Guardando recorrido…' : 'No se pudo guardar el recorrido en este dispositivo.'}</Text>
  ...
  return <View style={s.game}>
    <Scene race={race} paused={paused} />
    <View style={StyleSheet.absoluteFill} {...pan.panHandlers} />
    <View pointerEvents="box-none" style={[s.hud, { paddingTop: insets.top + 15 }]}>
      <View style={s.hudRow}><View accessibilityLabel={`${race.lives} vidas`} style={s.hearts}>{[0, 1, 2].map(i => <Text key={i} style={[s.heart, { color: i < race.lives ? '#e65f47' : '#c6d8db' }]}>♥</Text>)}</View><View style={s.scorePill}><Text style={s.score}>{race.score}</Text><Text style={s.scoreLabel}>PUNTOS</Text></View><Pressable accessibilityRole="button" accessibilityLabel="Pausar carrera" onPress={() => setPaused(true)} style={s.pause}><Text style={s.pauseIcon}>Ⅱ</Text></Pressable></View>
    ...
    <View pointerEvents="none" style={s.combo}><Text style={s.comboText}>{race.combo > 1 ? `${race.combo} ACIERTOS SEGUIDOS` : 'LEE · ELIGE · AVANZA'}</Text></View>
    <View style={[s.controls, { bottom: Math.max(17, insets.bottom + 8) }]}><Pressable accessibilityRole="button" accessibilityLabel="Mover a la izquierda" disabled={race.lane === 0} onPress={() => steer(-1)} style={({ pressed }) => [s.arrowButton, { opacity: race.lane === 0 ? .35 : pressed ? .65 : 1 }]}><Text style={s.arrow}>←</Text></Pressable><Text style={s.controlHint}>Desliza para{ '\n'}cambiar de carril</Text><Pressable accessibilityRole="button" accessibilityLabel="Mover a la derecha" disabled={race.lane === 2} onPress={() => steer(1)} style={({ pressed }) => [s.arrowButton, { opacity: race.lane === 2 ? .35 : pressed ? .65 : 1 }]}><Text style={s.arrow}>→</Text></Pressable></View>
