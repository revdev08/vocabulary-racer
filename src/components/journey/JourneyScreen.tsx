  ...
  return <View style={{ flex: 1, backgroundColor: paper }}>
    ...
    <SectionList showsVerticalScrollIndicator={false} ref={list} sections={units} keyExtractor={item => item.id} extraData={{ records, selectedLevelId, currentLevelId }} stickySectionHeadersEnabled={false} removeClippedSubviews={false} initialNumToRender={8} maxToRenderPerBatch={stress ? 48 : 8} windowSize={7}
      ...
      onScrollToIndexFailed={info => {
        if (!pending.current || failures.current++ >= Math.max(12, Math.ceil(units.reduce((count, unit) => count + unit.data.length + 2, 0) / 4))) { pending.current = null; return; }
      ...
      ListFooterComponent={<View style={{ padding: 18, alignItems: 'center' }}>{!stress && unitCount < journeyUnits.length ? <JourneyAction title="Ver más unidades →" onPress={() => setUnitCount(count => count + 1)}/> : <Text style={{ color: '#66726E', fontSize: 11 }}>FIN DEL RECORRIDO · SIGUE PRACTICANDO</Text>}</View>}/>
