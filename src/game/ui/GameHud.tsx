    ...
    <View style={styles.topRow}>
      <View accessible accessibilityLabel={`${view.coinsCollected} monedas`} style={[styles.counterPlate, { width: counterWidth }]}>
        ...
        <View style={[styles.counterCopy, { paddingLeft: iconSize - 3 }]}>
          <Text numberOfLines={1} adjustsFontSizeToFit maxFontSizeMultiplier={1.1} style={styles.caption}>MONEDAS</Text>
      ...
      <Animated.View accessible accessibilityLabel={`Racha de ${view.streak}`} style={[styles.counterPlate, { width: counterWidth }, streakPulse]}>
        ...
        <View style={[styles.counterCopy, { paddingLeft: iconSize - 5 }]}>
          <Text numberOfLines={1} adjustsFontSizeToFit maxFontSizeMultiplier={1.1} style={[styles.caption, styles.streakCaption]}>RACHA</Text>
      ...
      <View accessible accessibilityLabel={`${view.lives} vidas`} style={[styles.hearts, { width: heartsWidth }]}>
        {Array.from({ length: gameplay.initialLives }, (_, i) => <HudHeart key={i} size={heartSize} filled={i < view.lives} />)}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  caption: { fontSize: 7, lineHeight: 9, fontWeight: '800', color: '#D4E5F2', letterSpacing: .35, textAlign: 'center' },
  ...
  nitroText: { color: '#D9F8FF', fontWeight: '900', fontSize: 7, lineHeight: 10, letterSpacing: .8 },
  subRow: { pointerEvents: 'none', height: 30, marginTop: 4, paddingRight: 52, flexDirection: 'row', alignItems: 'center', gap: 10 },
  score: { maxWidth: '46%', minWidth: 80, flexDirection: 'row', alignItems: 'center', gap: 5, height: 26, paddingHorizontal: 8, borderRadius: 6, borderBottomWidth: 2, borderBottomColor: '#0B223B', backgroundColor: '#153957' },
  scoreAccent: { width: 3, height: 12, borderRadius: 1, backgroundColor: '#90DDEF', transform: [{ skewX: '-13deg' }] },
  scoreCaption: { fontSize: 7, color: '#C9DDEB', fontWeight: '800', letterSpacing: .5 },
  scoreValue: { flexShrink: 1, color: palette.white, fontWeight: '900', fontSize: 16, lineHeight: 20, fontVariant: ['tabular-nums'] },
  ...
  progressRow: { flex: 1, minWidth: 0, height: 30, paddingHorizontal: 7, justifyContent: 'center', gap: 3, borderRadius: 6, backgroundColor: '#153957ED' },
  timerLabel: { color: '#E3F0FC', fontSize: 8, lineHeight: 10, fontWeight: '800' },
