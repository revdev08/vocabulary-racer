...
(async()=>{
  ...
  for(const name of ['backdrop','walls','roadside']) {
    const source=name==='roadside'?'assets/game/scenery/tree_planter.png':`assets/game/maps/sunset/${name}.png`;
    const im=CK.MakeImageFromEncoded(fs.readFileSync(source)),w=im.width(),h=im.height();
    ...
    report[name]={source,width:w,height:h,transparentFraction:clear/(w*h),maxAlpha,bounds:[left,top,right,bottom],lowerHalfTransparent};
    if(name==='walls') {
      report.walls.crest=[];
    ...
    if(name==='backdrop') {
      ...
      for(let y=800;y<=1300;y+=25) {
        ...
        for(const side of [-1,1])for(let x=w/2;x>0&&x<w;x+=side) {
          const i=(y*w+x)*4;
          // The inner curb is in cool dusk shadow; daytime thresholds jump past it.
          if(px[i]>85&&px[i+1]>80){edges.push(x);break;}
        }
  ...
  fs.writeFileSync('docs/city/maps/sunset-measurements.json',JSON.stringify(report,null,2)+'\n');
