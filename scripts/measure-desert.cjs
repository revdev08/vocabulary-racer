...
(async()=>{
  ...
  for(const name of ['backdrop','walls','roadside']) {
    const im=CK.MakeImageFromEncoded(fs.readFileSync(`assets/game/maps/desert/${name}.png`)),w=im.width(),h=im.height();
  ...
  fs.writeFileSync('docs/city/maps/desert-measurements.json',JSON.stringify(report,null,2)+'\n');
