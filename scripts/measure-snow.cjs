...
(async()=>{
  ...
  for(const name of ['backdrop','walls','roadside']) {
    const im=CK.MakeImageFromEncoded(fs.readFileSync(`assets/game/maps/snow/${name}.png`)),w=im.width(),h=im.height();
  ...
  fs.writeFileSync('docs/city/maps/snow-measurements.json',JSON.stringify(report,null,2)+'\n');
